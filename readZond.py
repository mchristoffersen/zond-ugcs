...
def read(file, loglevel="INFO"):
    ...
    ntrace = int((size-fd.tell())/(240 + 512*2)) # 240 + 512*2 is header+trace size
    if(ntrace != (size-fd.tell())/(240 + 512*2)):
        log.warning("Non-integer number of traces calculated: %.3f" % ((size-fd.tell())/(240 + 512*2)))
    else:
    ...
    log.info("%d%% of traces have RTK fix" % (100*np.sum(df["RTK Status"] == "ON")//ntrace))
